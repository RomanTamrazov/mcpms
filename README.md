...
В ответах постановщика от 23.09.2026 численные ориентиры Precision 0,7,
Recall 0,5 и горизонт 24 часа названы плановыми, а не жёстким конкурсным
отсевом. Мы всё равно используем их как внутренние пороги на временных тестах.
Подтверждённых исходов ОДС, ремонтов и графиков горячих работ в датасете не
будет, поэтому карточки и README моделей явно называют используемые
прокси-цели. Полный разбор данных и ответов приведён в
...
- единая дизайн-система, адаптивный интерфейс, светлая/тёмная темы и сворачиваемая боковая панель;
- глобальный поиск `Cmd/Ctrl + K`, быстрые действия и центр уведомлений с источником и временем события;
- раздел «Графики ППР и ТО»: 26 плановых строк ППР датчиков метана, помесячная нагрузка ТО/ТР и состав оборудования из переданных графиков на 2026 год;
...
- предпросмотр, проверка и локальный импорт реестра CSV; экспорт CSV/GeoJSON;
...
1. Войти как диспетчер через быструю демо-кнопку на странице входа.
2. На Dashboard открыть прогноз, изучить факторы риска и перейти к объекту на карте.
3. Принять решение «Направить бригаду» или создать черновик заявки из карточки объекта.
4. Сменить демо-роль на технического специалиста, принять заявку в работу и закрыть её с результатом.
5. Вернуться к роли диспетчера: журнал и аналитика покажут историю решения в текущем браузере.

Прямые ссылки на разделы и карточки поддерживают обновление страницы и
навигацию назад/вперёд. В GitHub Pages они работают через `404.html`-fallback.
Демо-аккаунты, заявки, журнал, пользовательский реестр и настройки хранятся
только в `localStorage` браузера. Реальные ML-прогнозы появляются при
доступности API; демонстрационные карточки явно обозначены как демо-срез.
